...
Note that we can simulate data with this structure, but the correlations will not be *exactly* what we ask for because of sampling variability. Thus, the betas under the single-predictor and two-predictor models will be *very* close, but not identical:
```{r}
sim_nocorr <- data.frame(mvrnorm(n=5000, mu=mu, Sigma=Sigma))
...
sim_withcorr <- data.frame(mvrnorm(n=5000, mu=mu, Sigma=Sigma))
