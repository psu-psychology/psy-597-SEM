...
p_load(knitr, ppcor, tidyverse, lavaan, modelr, mlbench, lattice, lm.beta, semPlot, DiagrammeR)
...
## Decomposing variance explained in regression using (standardized coefficients, part, or partial correlations)
Determining the 
```{r}
#some work on relative importance in multiple regression, nonequivalence of R2 measures with standardized betas
#m5 <- lm(cmedv ~ log_crim + nox -1, BostonSmall_std)
#summary(m5)

m5 <- lm(cmedv ~ log_crim + nox, BostonSmall)
lm.beta(lm(cmedv ~ log_crim + nox, BostonSmall))

BostonSmall <- BostonSmall %>% add_predictions(m5)
coef(m5)["log_crim"] * cov(BostonSmall_std$cmedv, BostonSmall_std$log_crim)
coef(m5)["nox"] * cor(BostonSmall_std$cmedv, BostonSmall_std$nox)
library(relaimpo)
xx <- calc.relimp(m5, type = c("lmg", "first", "last", "betasq", "pratt", "genizi", "car"))
...
sim_nocorr <- data.frame(mvrnorm(n=1000, mu=mu, Sigma=Sigma))
...
#add correlation of 0.2 between x1 and x2
Sigma <- tribble(
  ~y, ~x1, ~x2,
  1, 0.4, 0.4,
  0.4, 1, 0.6,
...
sim_withcorr <- data.frame(mvrnorm(n=1000, mu=mu, Sigma=Sigma))
names(sim_withcorr) <- c("y", "x1", "x2")

#now, semipartial and standardized beta are not equivalence
